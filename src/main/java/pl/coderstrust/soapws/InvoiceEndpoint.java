...
public class InvoiceEndpoint {
  ...
  private InvoiceWsRepository invoiceWsRepository;

  @Autowired
  public InvoiceEndpoint(InvoiceWsRepository invoiceWsRepository) {
    this.invoiceWsRepository = invoiceWsRepository;
  }
  ...
  public GetInvoiceResponse getInvoice(@RequestPayload GetInvoiceRequest request)
    ...
    response.setInvoice(invoiceWsRepository.findInvoice(request.getId()));
    return response;
  }
  ...
  public GetInvoicesResponse getInvoices(@RequestPayload GetInvoicesRequest request)
    ...
    if (!isNotNull(request.getSelectByDate())
      ...
      invoiceWsList = invoiceWsRepository.findInvoices();
    }
    if (isNotNull(request.getSelectByDate())) {
      ...
      invoiceWsList = invoiceWsRepository.findInvoicesFromDateToDate(fromDate, toDate);
    }
    if (isNotNull(request.getSelectByYearAndMonth())) {
      invoiceWsList = invoiceWsRepository.findInvoicesByYearAndMonth(
  ...
  public SetInvoiceResponse setInvoice(@RequestPayload SetInvoiceRequest request)
    ...
    if (isNotNull(request.getInvoiceAttributes())) {
      if (!isNotNull(request.getInvoiceAttributes().getInvoiceBodyWs())) {
        response.setId(invoiceWsRepository.removeInvoice(request.getInvoiceAttributes().getId()));
      }
      if (!isNotNull(request.getInvoiceAttributes().getId())) {
        response.setId(invoiceWsRepository.saveInvoice(request
            .getInvoiceAttributes().getInvoiceBodyWs()));
      }
      if (isNotNull(request.getInvoiceAttributes().getId())
          && isNotNull(request.getInvoiceAttributes().getInvoiceBodyWs())) {
        response.setId(invoiceWsRepository.updateInvoice(request.getInvoiceAttributes().getId(),
